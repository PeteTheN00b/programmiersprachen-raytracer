...
void World::createMaterial(std::string const& matName, Color const& color, float diffusive, float specular)
{
	//mats_.emplace(matName, std::make_shared<Material>(new Material{ color, diffusive, specular }));
	mats_[matName] = std::make_shared<Material>(Material{ color, diffusive, specular });
}
...
void World::createSphere(std::string const& matName, glm::vec3 const& centre, float radius)
{
	...
	shapes_.push_back(std::make_shared<Sphere>(
		Sphere{ "Sphere" + std::to_string(sphereCount_), m.get()->color, m.get()->diffusive, m.get()->specular,
		centre, radius}));
...
void World::createBox(std::string const& matName, glm::vec3 const& origin, float w, float h, float l)
{
	...
	shapes_.push_back(std::make_shared<Box>(
		Box{ "Box" + std::to_string(boxCount_), m.get()->color, m.get()->diffusive, m.get()->specular,
		origin, w, h, l}));
